...
if (!isset($_GET['id'])) {
  Header("Location: index.php?#packetsSection");
}else{

  if ($_GET['id'] == 1) {
    $pack_name = 'СУМКА ЗНАНИИ';
  ...
  if (isset($_GET['check1']) && $_GET['check1'] == 'true') {
    $checks1 = true;
    $price += 5000;
  }else{
    $checks1 = false;
  }
  if (isset($_GET['check2']) && $_GET['check2'] == 'true') {
    $checks2 = true;
    $price += 5000;
  }else{
    $checks2 = false;
  }
  if (isset($_GET['check3']) && $_GET['check3'] == 'true') {
    $checks3 = true;
    $price += 5000;
  }else{
    $checks3 = false;
    ...
    <script type="text/javascript">
      ...
      var payHandler = function () {
        ...
        widget.charge({ // options
          publicId: 'pk_4d8023509ff4c257144d510c5cc17',
          description: 'Оплата за багаж знании: <?php echo ucfirst($pack_name); ?>',
          amount: <?php echo $price ?>, //сумма
          currency: 'KZT',
          invoiceId: '1234567', //номер заказа
          accountId: $('#email').val(), //плательщик
        },
        function (options) { // success
          var postForm = {
            'email': $('#email').val(),
            'packet': <?php echo $_GET['id']; ?>,
            'plus1': <?php echo $_GET['check1']; ?>,
            'plus2': <?php echo $_GET['check2']; ?>,
            'plus3': <?php echo $_GET['check3']; ?>
          };
          ...
          $.ajax({ //Process the form using $.ajax()
              ...
              dataType  : 'text',
              success: function(msg) {
                yaCounter38127950.reachGoal('zayavka');
                swal({   
                  title: "Так держать!",   
        ...
        function (reason, options) { // fail
          swal("Возникла ошибка", "Попробуйте еще раз", "error");
        });
      };
